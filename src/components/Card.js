...
const Card = ({ orderToBeShipped, shipments, token }) => {
  ...
  const items = orderToBeShipped.map((item) => {
    return (
      <li key={item.sale_item_id}>
        <span className={`${item.quantity > 1 ? "highlight" : ""} quantity`}>{item.quantity} - </span>
        {item.item_name}
      </li>
    );
  });
  ...
  useEffect(() => {
    ...
    if (shipments) {
      shipments.data.map((test) => {
        if (parseInt(test.order_id) === orderToBeShipped[0].payment_id && test.status === "ready") {
  ...
  if (cannotProcess) {
    return (
      <div key={orderToBeShipped[0].sale_item_id} className={`order manualComplete`}>
        <span className="date">{orderToBeShipped[0].order_date.substring(0, orderToBeShipped[0].order_date.length - 12)}</span>
        <div>
        ...
        <div className="buttons">
          <button className="buttonRounded opacity">Purchase Postage</button>
  ...
  return (
    <div key={orderToBeShipped[0].sale_item_id} className={`order ${alreadyPurchased && "completeOrder"} ${complete ? "complete" : ""} ${checked ? "pdfReady" : ""} ${preorder ? "blankOut" : ""}`}>
      <span className="date">{orderToBeShipped[0].order_date.substring(0, orderToBeShipped[0].order_date.length - 12)}</span>
      <div className="name">
        <p>{orderToBeShipped[0].ship_to_name}</p>
        <p>{orderToBeShipped[0].ship_to_country}</p>
      ...
      <div className="buttons">
        <button className={`buttonRounded ${alreadyPurchased ? "opacity" : "purchase"}`} onClick={() => handleClick()}>
          {alreadyPurchased ? "Purchased" : "Purchase Postage"}
        </button>
        <button
          className={`buttonRounded ${alreadyPurchased ? "reprint" : "opacity"}`}
          disabled={added}
          onClick={() => {
            shipments.data.map(async (ship) => {
              // If the order id matches the payment id we have our shipment.
              console.log(ship);
              if (parseInt(ship.order_id) === orderToBeShipped[0].payment_id && ship.status === "ready") {
                const getShipmentInfo = await getShipment(ship.id);
                // if (getShipmentInfo.data.shipment.status === "ready") {
                // Update the store with the information for the shipment we want to print the label of.
                add(getShipmentInfo.data.shipment.postage_label_png_url);
        ...
        <button className={`buttonRounded ${alreadyPurchased ? "shipped" : "opacity"}`} onClick={handleClickShip}>
          Mark Shipped
        </button>
      </div>
    </div>
