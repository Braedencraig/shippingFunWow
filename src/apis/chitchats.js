...
export const createShipment = async(orderToBeShipped) => {
    try {
        let postageType = () => {
            if (orderToBeShipped[0].ship_to_country_code === "SI" || orderToBeShipped[0].ship_to_country_code === "HR" || orderToBeShipped[0].ship_to_country_code === "GR") {
        ...
        const res = await axios.post(`/clients/${chitChatClientId}/shipments`, shipmentBody, {
            headers: {
                Authorization: chitChatTkn,
            },
        });
